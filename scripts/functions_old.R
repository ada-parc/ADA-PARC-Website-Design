    ...
    if (!is_comp) {
      ...
      ggplot(data = us_states_with_data) +
        ...
        theme(
          ...
          legend.text = element_text(size = 8), # Slightly smaller font size for labels
          legend.title.align = 0.5,
          legend.box = "horizontal",
          legend.box.just = "center",
          legend.margin = margin(t = 10, b = 10), # Add extra space around the legend
          plot.margin = margin(t = 10, r = 10, b = 40, l = 10) # Extra space at the bottom
        ) +
        guides(
          fill = guide_legend(
            title.position = "top",
            ...
            keyheight = unit(1, "cm"), # Larger legend key height
            keywidth = unit(2, "cm") # Larger legend key width
          )
        )
      ...
      legend <- cowplot::get_legend(
        ggplot(data = us_states_with_data) +
          geom_sf(aes(fill = estimate_cat)) +
          scale_fill_manual(values = palette, drop = FALSE, name = "Legend Title") +
          theme(
            legend.position = "bottom",
            legend.direction = "horizontal",
