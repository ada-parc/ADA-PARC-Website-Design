...
# Transform data to spatial data
# Not used in national tile map version
...
# Create color palette for national data
# Not used in national tile map version
...
output$demo2map <- renderLeaflet({
  ...
  # Map
  render_tile_map(demo_age_map_data, demo_age_selected(), pal_demo2())
...
output$demo3map <- renderLeaflet({
  ...
  # Map
  render_tile_map(demo_race_map_data, demo_race_selected(), pal_demo3())
...
```{r demographics race plot, include=FALSE}
...
output$demo4map <- renderLeaflet({
  ...
  # Map
  render_tile_map(demo_gender_map_data, demo_gender_selected(), pal_demo4())
...
output$demo5map <- renderLeaflet({
  ...
  # Map
  render_tile_map(demo_distype_map_data, demo_distype_selected(), pal_demo5())
...
output$cl1map <- renderLeaflet({
  ...
  render_tile_map(cl_live_map_data, cl_live_selected(), pal_cl1())
...
output$cl2map <- renderLeaflet({
  ...
  render_tile_map(cl_nursing_map_data, cl_nursing_selected(), pal_cl2())
...
output$cl3map <- renderLeaflet({
  ...
  render_tile_map(cl_ps_map_data, cl_ps_selected(), pal_cl3())
...
output$cp1map <- renderLeaflet({
  ...
  # Map
  render_tile_map(cp_insurance_map_data, cp_insurance_selected(), pal_cp1())
...
output$cp2map <- renderLeaflet({
  ...
  render_tile_map(cp_housing_map_data, cp_housing_selected(), pal_cp2())
...
output$cp3map <- renderLeaflet({
  ...
  render_tile_map(cp_transit_map_data, cp_transit_selected(), pal_cp3())
...
output$cp4map <- renderLeaflet({
  ...
  # Map
  render_tile_map(cp_crime_map_data, cp_crime_selected(), pal_cp4())
...
output$cp5map <- renderLeaflet({
  ...
  render_tile_map(cp_education_map_data, cp_education_selected(), pal_cp5())
...
output$cp6map <- renderLeaflet({
  ...
  render_tile_map(cp_voting_map_data, cp_voting_selected(), pal_cp6())
...
output$we1map <- renderLeaflet({
  ...
  render_tile_map(we_employ_map_data, we_employ_selected(), pal_we1())
...
output$we2map <- renderLeaflet({
  ...
  render_tile_map(we_status_map_data, we_status_selected(), pal_we2())
...
output$we3map <- renderLeaflet({
  ...
  render_tile_map(we_afford_map_data, we_afford_selected(), pal_we3())
...
metro_snapshot_place_full <- pull_mongo_data(tables, "place")
rm(tables)
...
renderUI({
  ...
  metro_snap_overview_text <- paste0(
    ...
    "Based on ACS 2018 5-Year Estimates, <b>", metro_snapshot_place() %>% pull(estimate_S1810_C02_001) %>% scales::comma(.), "</b> of the city's <b>", metro_snapshot_place() %>% pull(estimate_S1810_C01_001) %>% scales::comma(.), "</b> residents (<b>", metro_snapshot_place() %>% pull(estimate_S1810_C03_001), "%</b>) are persons with disabilities.<br>")
...
output$metro_snapshot_we_plot <- renderPlotly({
  ...
  gg <- metro_snapshot_place_full %>%
    select(GEOID,
           NAME,
           "pwd_employed" = estimate_S1811_C02_002,
