...
output$national_map_summary <- renderUI({
  ...
  if(national_variable_selected %in% hud_data) {
      source_text <- "2021 United States Department of Housing and Urban Development"

  } else {
      source_text <- "2017-2021 American Community Survey five-year estimates"
...
output$city_summary <- renderUI({
  ...
  city_summary_text <- paste0(
    ...
    city_place() %>% 
      ...
      pull(NAME), "</h4>",
    "Based on ACS 2021 5-Year Estimates, <b>", 
    city_place() %>%
      pull(pwd_total),
    "</b> of the city's <b>",
    city_place() %>%
  ...
  city_summary_demo <- city_place() %>%
    mutate(
      ...
      "Other" = create_number_percent_string(pwd_other, pwd_other_dis_pct),
      "<b>Total" =
        paste0(pwd_total,
               "</b>"),
      .keep = "none"
  ...
  city_summary_cp <- city_place() %>%
    mutate(
      ...
      "Transit" =  create_number_percent_string(pwd_pub_transit, pwd_pub_transit_dis_pct),
      "Walk or Bike" = create_number_percent_string(pwd_walk_bike, pwd_walk_bike_pct),
      "Work from Home" = create_number_percent_string(pwd_wfh, pwd_wfh_dis_pct),
      "<b>Total" =
