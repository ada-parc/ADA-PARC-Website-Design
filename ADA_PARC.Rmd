...
###### Jane Test
# old_tracts_data <- tracts_data
# old_city_place_full <- city_place_full
# 
# 
#   
# tracts_data <- readRDS(here::here("data", "tract_data.Rds")) %>% filter(year == 2021)
# 
# city_place_full <- readRDS(here::here("data", "city_place_full.Rds")) %>% filter(year == 2021)
...
# dict_location_crosswalk2 <- dict_location_crosswalk %>%
...
output$city_summary <- renderUI({
  ...
  city_summary_demo <- city_place() %>%
    select("White" = pwd_white_nonhisp,
           "Black" = pwd_black,
           "Hispanic" = pwd_hisp,
           "Asian" = pwd_asian,
           "Other" = pwd_other,
  ...
  city_summary_cp <- city_place() %>%
    transmute("Car" =
                paste0(comma(pwd_car_commute,
                             accuracy = 1),
                       " (", comma((pwd_commute_car_alone_pct +
                                      pwd_commute_carpool),
                                   accuracy = 0.1), "%)"),
              "Transit" =
                paste0(comma(pwd_total_commute *
                               (pwd_commute_public_pct / 100),
                             accuracy = 1),
                       " (", comma(pwd_commute_public_pct,
                                   accuracy = 0.1), "%)"),

              "Walk or Bike" =
                paste0(comma(pwd_total_commute *
                               ((pwd_commute_walk +
                                   pwd_commute_taxi_car_bike_etc) / 100),
                             accuracy = 1),
                       " (", comma((pwd_commute_walk +
                                      pwd_commute_taxi_car_bike_etc),
                                   accuracy = 0.1), "%)"),
              "Work from Home" =
                paste0(comma(pwd_total_commute *
                               (pwd_grtoeq_16_wfh_pct / 100),
                             accuracy = 1),
                       " (", comma((pwd_grtoeq_16_wfh_pct),
                                   accuracy = 0.1), "%)"),
              "<b>Total" =
                paste0(comma(pwd_total_commute,
                             accuracy = 1),
  ...
  city_summary_we <- city_place() %>%
    transmute("Employed" =
                paste0(comma(pop_total_employed_16_plus, accuracy = 1),
                       " (", pwd_employed, "%)"),
              "Not in Labor Force" =
                paste0(comma((pwd_16_plus - pop_total_employed_16_plus), accuracy = 1),
                       " (", pwd_not_employed, "%)"),
              "<b>Total" =
                paste0(comma(pwd_16_plus, accuracy = 1),
                       "</b>")) %>%
    pivot_longer(everything(),
...
<!--     select(GEOID, -->
<!--            NAME, -->
<!--            "pwd_employed" = pop_total_employed_16_plus, -->
<!--            "pwd" = S1811_C02_001_estimate, -->
<!--            "pwd_employed_pct" = pwd_employed) %>%  -->
<!--     ggplot(aes(text = paste0("City: ", NAME,  -->
<!--                              "<br>PWD Employed: ",  -->
