...
# Set national df by input$national_category_selector
# Could eval as symbol if the same name (e.g. df name and input `demographics`),
# returning the object itself, but we want to preserve df names by section
# and the input value is needed to limit dict_vars matches
national_data <- reactive({
  
  if (input$national_category_selector == "demographics"){
    national_demographics
  } else if (input$national_category_selector == "community_participation"){
    national_participation
  } else if (input$national_category_selector == "community_living"){
    national_living
  } else {
    national_economic
  }
