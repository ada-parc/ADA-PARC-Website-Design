...
library(scales); library(hrbrthemes); library(ggthemes); library(extrafont)
...
Overview {data-navmenu="By Metro"}
======================================================================
...
# Define inputs
# Metro
selectizeInput(inputId = "metro_select", 
...
```{r metro overview table}

DT::renderDataTable({
  
  if(is.null(input$metro_select)){
    DT::datatable(
      metro_demo_data %>% 
    ...
    DT::datatable(
      metro_demo_data %>% 
        filter(metro_state %in% input$metro_select) %>% 
        select("City" = metro_state, input$var_select),
      options(list(scrollX = "200px", 
                   pageLength = 5,
                   lengthMenu = list(c(5, 10, 20, -1), c("5", "10", "20", "All"))))
    )
...
output$metro_plot <- renderPlot({
  ...
  req(input$metro_select)
  req(!is.null(input$var_select))
  
  # Plot
  metro_demo_data %>%
    filter(metro_state %in% input$metro_select) %>%
    select(metro_state, input$var_select) %>% 
...
if(is.null(input$comp)){
    ...
    DT::datatable(
      metro_demo_data %>% 
        filter(metro_state %in% input$comp) %>% 
