...
renderText({ is.null(input$national_variable_selector) })
renderText({ national_category_selected() })
renderText({ input$national_variable_selector })

# Update select dropdown with named list of values created below
observeEvent(input$national_category_selector, {
  
  # Create reactive named list of choices based on national_category_selector
  national_variable_choices <- tibble::deframe(
    dict_vars %>% 
  ...
  # Default topic choice depending on category
  # national_variable_default <- 
  #   if (temp_category == "is_community_participation") {
  ...
  dict_vars %>% 
    filter((!!sym(input$national_category_selector)) == TRUE,
           var_topic ==
             (dict_vars %>% 
                filter((!!sym(input$national_category_selector)) == TRUE,
                       var_readable == input$national_variable_selector) %>% 
                pull(var_topic)),
           display == T) %>% 
...
table_caption <- reactive({
  ...
  paste(names(cap_choices[cap_choices == input$national_category_selector]), "Table")
...
```{r download fact sheets sidebar}
...
/*
#access_map_palette ~ .selectize-control .option[data-value="Yellow-Orange-Brown"]{ 
background-image: linear-gradient(to right, white, #ffffd4, #fed98e, #fe9929, #cc4c02) !important;
background-size: auto !important;
}

#access_map_palette ~ .selectize-control .option[data-value="Yellow-Orange-Red"]{ 
background-image: linear-gradient(to right, white , #ffffb2, #fecc5c, #fd8d3c, #e31a1c) !important;
background-size: auto !important;
}

#access_map_palette ~ .selectize-control .option[data-value="Green-Blue"]{ 
