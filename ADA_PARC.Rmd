...
```{r national_sidebar, context="server", message=FALSE, warning=FALSE, fig.alt = "A sidebar containing two drop down menus. One selects a category, and the other selects a topic within that category. These other elements of this page will change based on the selected category and topic."}
...
selectInput(inputId = "national_category_selector", 
            ...
            selected = "is_community_participation",
            multiple = FALSE,
            selectize = TRUE)
...
selectInput(inputId = "national_variable_selector",
            ...
            choices = tibble::deframe(
              dict_vars %>%
                filter(is_community_participation == TRUE,
                       !is.na(national_dropdown_label)) %>% #
                select(national_dropdown_label, var_readable)),
            selected = "pwd_commute_car_alone_pct",
            multiple = FALSE,
            selectize = TRUE)
...
observeEvent(input$national_category_selector, {
  ...
  temp_category <- input$national_category_selector
   if (is.null(temp_category)) temp_category <- "is_community_participation"
   
  # Create reactive named list of choices based on national_category_selector
  national_variable_choices <- tibble::deframe(
    dict_vars %>% 
      filter((!!sym(temp_category)) == TRUE,
             var_readable %in% (
               national_data() %>%
  ...
  updateSelectInput(session,
                    ...
                    selected = national_variable_default)
...
# UI Output
renderUI({ HTML(national_topic_table()) })
