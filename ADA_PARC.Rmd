...
dict_vars <-  fun_pull_mongo_data(tables = "vars_dict", host_name = "host_prod")
```
...
National Data {data-navmenu="National" data-icon="fa-user"}
======================================================================
...
# State Lookup
mongo_conn_input <- fun_mongo_connect(host_name = "host_dev",
                                      collection_name = "acs_state_S1810",
                                      database_name = "ADA-PARC")

national_input <- mongo_conn_input$find(fields = '{"GEOID" : true, "NAME" : true, "state" : true}')

# Define inputs
selectInput(inputId = "category", 
            label = "Select a category", 
            choices = c("Demographics" = "national_demographic", # input$category returns the name of the table
                                                                 # and that is used to pull the list of variables
                                                                 # using dict_vars
                        "Community Participation" = "national_participation", 
                        "Community Living" = "national_living", 
                        "Work/Economic" = "national_economic"))

selectInput(inputId = "variable",
            label = "Select a variable",
            choices = c()) # TODO: Build out reactivity to category input
            # Below should pull the list of variables in database code format
            # choices = get(input$category) %>% names() %>% str_extract(pattern = "[:alnum:]*_[:alnum:]*_[:alnum:]*") %>% unique() %>% na.omit()

# ====================
# Import national data
# ====================

# TODO: Make data import reactive to cat/var inputs

### Demographics
tables <- c("S1810")
national_demographic <- fun_pull_mongo_data(tables, host_name = "host_dev", "state")

### Community Living
tables <- c("S2601A", "S2602", "S1810")
national_living <- fun_pull_mongo_data(tables, host_name = "host_dev", "state")

### Community Participation
tables <- c("B18135", "S1811", "S1810") # TODO: B18135 is not joining with the other tables
national_participation <- fun_pull_mongo_data(tables, host_name = "host_dev", "state")

### Work/Economic
tables <- c("B18135", "B18140", "C18120", "C18121", "C18130", "S1811", "B25091", "B25070") # TODO: Multiple problem tables in this set as well
national_economic <- fun_pull_mongo_data(tables, host_name = "host_dev", "state")
rm(tables)
```
...
# Selected data
# TODO: Dynamic based on inputs in sidebar
demo_gen_data <- national_demographic %>% 
  filter(is.na(city)) %>% 
...
demo_gen_selected <- reactive({ "percent_of_total_population_with_a_disability" }) # TODO: Reactive to new inputs in sidebar
...
# TODO: Make this possible (plotly?)
...
metro_snapshot_place_full <- fun_pull_mongo_data(tables, "place")
rm(tables)
