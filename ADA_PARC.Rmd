...
Use the **Download Data** tab to get the raw data powering this dashboard.

Use the **Download Fact Sheets** tab to get pre-formatted fact sheets for a specific area of interest.
...
national_category_choices <- c("Demographics" = "is_demographics", 
...
national_category_selected <- reactive({ # Output looks like "is_demographics"
  getElement(national_category_choices, input$national_category_selector)
  # national_category_choices[[input$national_category_selector]]
})

# National variable selection widget
...
observeEvent(input$national_category_selector, {
    ...
    updateSelectInput(session, "national_variable_selector", choices = choices)
  })

national_variable_selected <- reactive({ # Output looks like "is_demographics"
...
output$national_map_summary <- renderUI({
  ...
  HTML(
    paste0(
      ...
      " Detailed information is available below in the interactive map (top) and table (bottom)."
    )
    # # Static check
...
<!-- <div role="region" aria-labelledby="section-national-summary"> -->
...
```{r national-map, fig.cap= "A map of the United States, with each state shaded according to the quartiles described in the summary.", fig.alt= "A map of the United States, with each state shaded according to the quartiles described in the summary."}

# # Button to choose map type
# radioButtons(inputId = "national_map_toggle",
#              label = "Select a map type to display:",
#              choices = c("Geographic" = "geographic",
#                          "Tile" = "tile"),
#              selected = "geographic",
#              inline = TRUE)

# Conditional UI depending on button selection
output$national_map <- renderUI({
  
  # if(input$national_map_toggle == "geographic") {
    withSpinner(tmapOutput("geographic_map") #%>%
  #                 # Adding the `alt` attribute to our plot
                  # tagAppendAttributes(alt = "A map of the United States, with each state shaded according to the quartiles described in the summary.")
                  )
  # } else {
  #   withSpinner(plotOutput("tile_map", height = "300px"))
  # }
...
# Geographic map
output$geographic_map <- renderTmap({
  
  # req(input$national_variable_selector)
  
  render_geo_interactive_map(national_data(), 
                             national_variable_selected(), 
                             access_map_palette_selected())
...
# UI Output
output$national_table <- renderUI({ HTML(national_topic_table()) })
...
city_place <- reactive({
  ...
  city_place_full %>% 
    filter(GEOID == input$city_selector)
...
city_tract_sf <- reactive({
  ...
  temp_city <- dict_location_crosswalk %>% 
    filter(place_GEOID %in% c(input$city_selector)) %>% 
    pull(tract_GEOID)
...
city_tract <- reactive ({
  ...
  temp <- temp_mongo_conn$find(
    query = sprintf('{ "GEOID" : { "$in" : [  "%s" ] } }' ,
                    str_c(city_tract_sf()$GEOID, collapse = '", "')))
  return(temp)
...
output$city_overview <- renderUI({
  
  # Overview text
  city_overview_text <- paste0(
  ...
  city_overview_tables <- knitr::kables(list(
  ...
  # UI Output
  HTML(paste(city_overview_text, city_overview_tables, sep = "<br>"))
  
})

withSpinner(uiOutput("city_overview"))
