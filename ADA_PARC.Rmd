...
metro_name <- data.frame(city = demo_metro_data$city, state = demo_metro_data$state_abbv, full = paste(demo_metro_data$city, ",", demo_metro_data$state_abbv))
...
### Make DT respond to the selectInputs above

### Implement GGPlot of the two selected items in the Interactive Viz box
...
DT::renderDataTable({
  ...
  if(is.null(input$metro_select) | is.null(input$var_select)){ ### If one of the two sidebar selections have not been made
    ### I would like it so that each of the selections will effect the output separately, and that both aren't needed for the output to be changed
      DT::datatable(
    demo_metro_data %>% 
      rename("percent_pwd" = percent_of_total_population_with_a_disability,
             "total_pop" = total_population) %>% 
      mutate(percent_pwd = percent(percent_pwd, scale = 1, accuracy = 0.1),
             ...
             total_pop = comma(total_pop)),
    options(scrollX = "200px"))
  } else { ### When both are selected, it filters the table down to the selected metros and the selected variable.
     DT::datatable(
    demo_metro_data %>% 
...
```{r }
### 
# I'm thinking that we implement a scatterplot here with labeled points. Organizing it by x = population total, y = pwd possibly
### Output:
# ggplot bar plot with the y values of the selected variable and the x values of the selected cities
