...
```{r participant-logos, echo = FALSE, fig.cap = "ADA-PARC Participants", out.width = '75%'}

knitr::include_graphics("./participant_logos.png")
...
output$metro_snapshot_overview <- renderUI({
  ...
  metro_snap_cond_text <- if (is.null(input$active_tab) || input$active_tab == 0) {
    ...
    kbl(x = metro_snapshot_place() %>%
        ...
        caption = "Commuting Patterns, People with Disabilities, Workers 16+",
        escape = FALSE,
        align = "c",
        table.attr = "style='width:100%;'") %>% 
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
    ...
    kbl(x = metro_snapshot_place() %>%
        ...
        caption = "Employment Status, People with Disabilities",
        escape = FALSE,
        align = "c",
        table.attr = "style='width:100%;'") %>% 
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
...
```{r metro-snapshot-overview-text-output}
...
```{r metro-snapshot-overview-map-output} 
...
# Plot
  gg <- metro_snapshot_place_full %>%
    ...
    ggplot(aes(text = paste0("City: ", NAME, 
                             "<br>PWD Bachelor's or<br>Higher: ", pwd_bachelors, "%",
                             "<br>PWD 25 or<br>Older", scales::comma(pwd_25_over)))) +
    geom_point(stat = "identity",
               aes(x = pwd_25_over, y = pwd_bachelors)) +
    gghighlight(GEOID == input$metro_snapshot) +
    labs(x = "People with Disabilities, Age 25 or Older", y = "Bachelor's Degree or Higher (%)",
         title = "Bachelor's Degree or Higher (%) vs. PWD 25 or Older") +
    scale_x_log10(labels = scales::comma) +
    scale_y_continuous(labels = function(x) paste0(x, "%")) +
    ggplot_fill_selected() +
    ggplot_theme_selected() +
...
```{r metro-snapshot-plots-output, out.height="310px", out.width="600px"}

# Tab panel output
tabsetPanel(type = "tabs",
            tabPanel("Demographics",
                     plotlyOutput("metro_snapshot_demo_plot", height = "310px")),
            tabPanel("Community Participation",
                     plotlyOutput("metro_snapshot_cp_plot", height = "310px")),
...
```{r metro-snapshot-tract-table, context="server"}
...
# Table
output$metro_snapshot_tract_table <- renderUI({ HTML(
...
```{r metro-snapshot-tract-table-output, out.height="310px", out.width="600px"}

uiOutput("metro_snapshot_tract_table")
