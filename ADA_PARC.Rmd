...
metro_snapshot_tract_sf <- reactive({
  ...
  temp_metro <-  list(
    temp_metro = case_when(input$metro_snapshot == "Arlington Heights Village, IL" ~
                             "Arlington Heights, IL",
                           input$metro_snapshot == "Hazel Crest Village, IL" ~
                             "Hazel Crest, IL",
                           input$metro_snapshot == "Indianapolis, IN" ~
                             "Indianapolis city (balance), IN",
                           input$metro_snapshot == "Naperville City, IL" ~
                             "Naperville, IL",
                           input$metro_snapshot == "Nashville-Davidson, TN" ~
                             "Nashville-Davidson metropolitan government (balance), TN",
                           TRUE ~ input$metro_snapshot))

  # Connect and pull
  temp_mongo_conn <- mongo_connect(collection_name = "geo_tract",
                                   database_name = "ADA-PARC")
  temp_geo <- temp_mongo_conn$iterate(query = sprintf('{ "properties.metro_state" : "%s" }',
                                                      temp_metro))
...
metro_snapshot_tract <- reactive({
  ...
  semi_join(metro_snapshot_tract_full, metro_snapshot_tract_sf(), by = "GEOID")
...
### City in Context {.data-width=500}

```{r metro snapshot plots}
...
output$metro_snapshot_demo_plot <- renderPlotly({
  ...
  gg <- metro_demo_data %>%
    ...
    mutate("pwd_perc" = coalesce(pwd_perc, 
                                 round((pwd_perc / total_population) * 100, 1))) %>% 
...
output$metro_snapshot_we_plot <- renderPlotly({
  ...
  gg <- metro_we_data %>%
    ...
    labs(x = "People with Disabilities", y = "PWD Unemployment Rate (%)",
         title = "PWD Unemployment Rate (%) vs. PWD Population") +
    scale_x_log10(labels = scales::comma) +
    scale_y_continuous(labels = function(x) paste0(x, "%")) +
    ggplot_fill_selected() +
    ggplot_theme_selected() +
...
### Characteristics of Selected Tract (Click on Map to Show Table) {.table-wrapper .data-width=500}
...
renderUI({ HTML(
  ...
  kbl(metro_snapshot_table_data() %>%
        mutate(output = paste0(comma(estimate), " (±", comma(moe), ")")) %>% 
        mutate(across(everything(), 
                      ~cell_spec(., "html",
                                 color = ifelse(moe >= estimate, "red", "black")))) %>%
        select("Variable" = label, "Est." = output),
      escape = FALSE) %>% 
