...
# save(city_place_full, community_living, community_participation, demographics, dict_location_crosswalk, dict_vars, tracts_data, tracts_sf, work_economic, file = here::here("data", "all.rda"))
...
# source(here("scripts", "national_import.R"), local = TRUE)

# City-level data

# city_place_full <- read_csv(here::here("data", "city_place.csv"))
# 
# tracts_sf <- sf::st_read(here::here("data", "tracts_export_geography.shp"))

# tracts_data <- read_csv(here::here("data", "tracts_export_data.csv")) %>%
#   pivot_wider(
...
# load(here::here("data", "tracts_data.RData"))

# Pull location crosswalk dictionary from GitHub repo
# dict_location_crosswalk <- read_csv("https://raw.githubusercontent.com/ada-parc/ADA-PARC-Website-Design/master/dictionaries/dict_location_crosswalk.txt") %>%
#   filter(tract_GEOID %in% tracts_sf$t_GEOID)

# tracts_lu <- read_csv(here::here("data", "tracts_export_lookup.csv"))
```
...
output$national_map_summary <- renderUI({
  ...
  HTML(
    paste0(
      ...
      " Detailed information is available to the right in the interactive map (top) and table (bottom).")
  )
...
To explore the data select a city below. You can navigate the box using the mouse or keyboard. For keyboard users, once the box is highlighted you can press backspace and begin typing the selection into the box.
...
output$city_summary <- renderUI({
  ...
  city_summary_text <- paste0(
    ...
    city_place() %>% 
      ...
      pull(NAME), "</h4>",
    "Based on ACS 2019 5-Year Estimates, <b>", city_place() %>% 
