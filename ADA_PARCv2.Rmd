...
# Create a heatmap visualization for the 'INDEX' column
sheet2 %>%
  mutate(INDEX = cell_spec(INDEX, "html", bold = TRUE, color = "white",
                           background = spec_color(INDEX, end = 0.7, option = "D"))) %>%
  kable("html", escape = FALSE, caption = "Index - Community Living ") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))


# Prepare US states map data
states_map <- map_data("state")

# Merge the map data with your sheet2 data
sheet2$State <- tolower(sheet2$State)
states_map <- states_map %>%
  left_join(sheet2, by = c("region" = "State"))

# Create a ggplot heatmap for the 'INDEX' column
#community_index_plot <- ggplot(states_map, aes(x = long, y = lat, group = group, fill = INDEX)) +
  #geom_polygon(color = "white") +
  #scale_fill_gradient(low = "lightblue", high = "blue", na.value = "grey50") +
  #theme_minimal() +
  ...
  #labs(title = "Heatmap of INDEX Values by State", fill = "INDEX")


# Display the plot
#print(community_index_plot)
```
