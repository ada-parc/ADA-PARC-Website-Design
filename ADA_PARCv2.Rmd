...
Application authors: <br> Ethan Jantz, Sean Connelly, Jane Huber,Colin Heeneman, and
Randy D. Colón <br> [UIC Voorhees
...
**Since the ADA**, public and private organizations have worked tirelessly to 
collect, interpret, and analyze data which sheds light on the quality of life
for individuals with a disability in the United States. 

<br>

**ADA PARC** highlights key components of Census and HUD data related to disability
outcomes to better elucidate the realities on the ground. 
...
*Illinois is the state with the highest percentage of its disabled population age 18-64 living in a nursing home.*
...
Scorecards for ADA-PARC translate measures across the three categories of Community Living, Community Participation, and Work Economic into an easy to interpret, comparative 100-point scale.

[Download PDF Report](../Documents/GitHub/ADA-PARC-Website-Design/ADA-PARC_scorecard_report_pdf.pdf)

There are two basic ways we can think about the ‘state of affairs’ for the disabled population in any given state x for metric y:

1. How does disabled population in state x fare on metric y compared to disabled population in other states? (relative)
2. How does disabled population ins tate x fare on metric y compared to non-disabled population in own state? (disparity/equity)

Relative scores indicate how favorable conditions are for a given state’s disabled population compared to the disabled population of other states, the higher the value, the more favorable conditions for that state's disabled population relative to the median state's disabled population.

Equity scores measures how favorable conditions are for a given state’s disabled population relative to the non-disabled population of the same state, the higher the value, the more favorable conditions are for that state's disabled population relative to the non-disabled population of that state. 
...
relative_community_living_data <- read_excel("between_states/between_states_relative_data/community_living_between_2022_excel.xlsx")
relative_community_participation_data <- read_excel("between_states/between_states_relative_data/community_participation_between_2022_excel.xlsx")
relative_work_economic_data <- read_excel("between_states/between_states_relative_data/work_economic_between_2022_excel.xlsx")

# Load equity datasets
equity_community_living_data <- read_excel("within_states/within_state_disparity_data/community_living_within_2022_excel.xlsx")
equity_community_participation_data <- read_excel("within_states/within_state_disparity_data/community_participation_within_2022_excel.xlsx")
equity_work_economic_data <- read_excel("within_states/within_state_disparity_data/work_economic_within_2022_excel.xlsx")
...
shinyApp(
  ...
  server = function(input, output, session) {
    ...
    output$scorecard_output <- renderUI({
      ...
      if (category == "relative_community_living") {
        tags$iframe(src = "https://your-server.com/views/relative_community_living", 
                    style = "width:100%; height:600px;")
      } else if (category == "relative_community_participation") {
        tags$iframe(src = "https://your-server.com/views/relative_community_participation", 
                    style = "width:100%; height:600px;")
      } else if (category == "relative_work_economic") {
        tags$iframe(src = "https://your-server.com/views/relative_work_economic", 
                    style = "width:100%; height:600px;")
      } else if (category == "equity_community_living") {
        tags$iframe(src = "https://your-server.com/views/equity_community_living", 
                    style = "width:100%; height:600px;")
      } else if (category == "equity_community_participation") {
        tags$iframe(src = "https://your-server.com/views/equity_community_participation", 
                    style = "width:100%; height:600px;")
      } else if (category == "equity_work_economic") {
        tags$iframe(src = "https://your-server.com/views/equity_work_economic", 
                    style = "width:100%; height:600px;")
      }
    })
  }
...
2.  **To identify & examine key environmental factors** contributing to
    these disparities
3.  **To benchmark participation disparities and highlight promising practices** 
    at state & city levels
4.  **To action-plan strategies for dissemination and utilization of  findings**
    to be used by ADA Centers and others in community capacity building & systems
    change initiatives
